/* $Id: ExecutiveHelper.java,v 1.35 2004/05/04 02:10:12 tk1748 Exp $ */
package mud;
...
public final class ExecutiveHelper
{
    ...
    */
	...
	if (keyAction.getSocket() == null)
	{
		...
		thisStage.getGameValue().propCounter =
		    new Text(new RealPos(700,550),thisStage,1,"");
		thisStage.getGameValue().frameCounter =
		    new Text(new RealPos(700,530),thisStage,1,"");
		thisStage.getGameValue().fpsCounter =
		    new Text(new RealPos(700,540),thisStage,1,"[FPS]");
	    }
	    else
	    {
    ...
    */
	...
	case 1:
	    ...
	    break;
	case 2:
	    lastProp = new WaterDrip(new RealPos(-100,-100), thisStage);
	    break;
	case 3:
	    lastProp = new Deformo(new RealPos(-100,-100), thisStage);
	    break;
	case 4:
	    lastProp = new Stream(new RealPos(-100,-100), thisStage);
	    break;
	default:
    ...
    public void speedHelper(Integer w)
    {
	if (lastProp!=null)
	{
	    lastProp.setSpeed(i(w));
	}
	else
	{
	    System.err.println("speed must be used after an object");
	}
    }
    ...
    */
	...
	/* This is the default position for a new player */
        Player t = new Player(new RealPos(-150,200), thisStage, type);
	t.setName(w);
	currentPlayer = t;
	players.add(t);
